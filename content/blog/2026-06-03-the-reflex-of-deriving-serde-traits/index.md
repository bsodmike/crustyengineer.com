...
This is a tiny addendum to those new to Rust, and may be confused by the `DomainObject` type; it's mentioned in the article but not referenced by any code.

You will however find its use in the actual code linked where it's used to implement `From` for type conversions. This is made clear in the example source itself
...
This lets you call `.into()`. The only reason this is cloned below is simply due to being used twice and the `From` conversion is only implemented on an owned value.
